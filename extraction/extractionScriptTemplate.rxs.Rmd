...
################################################################################
  ...
  ##############################################################################
    ...
    ############################################################################
      ...
      ##########################################################################
      ############################################ START: operationalisation ###
      ##########################################################################
      study$methods$variables$AddChild('operationalisation');
      study$methods$variables$operationalisation[['value']] <-
      ##########################################################################
      ### 
      ### OPERATIONALISATION
      ### 
      ...
      ##########################################################################
      study$methods$variables$operationalisation[['validation']] <- list(`oper.name` = expression(!is.na(VALUE) && !is.null(VALUE) && (nchar(VALUE) > 0)),
  ...
  ##############################################################################
    ...
    ############################################################################
        ...
        list(assoc.name = "Enter name here (mandatory)",    ### Name of result: A unique name (in this study), to easily find / meta-analyse / group this result later [Examples: "mainHypothesisTest"; "manipulationCheck"] [Value description: A single character value that cannot be omitted]
             assoc.variable1 = "enter <<entityRef>> here",  ### First variable: The name of the first variable. [Examples: <no example>] [Value description: A string that specifies another entity and which MUST be provided]
             assoc.var1name = NA,                           ### Values of first variable: Values this variable pertains to (e.g. in post hoc analyses). [Examples: <no example>] [Value description: A string that specifies another field in another entity (can be missing, i.e. NA).]
    ...
    study$results$association__1__[['validation']] <- list(`assoc.name` = expression(!is.na(VALUE) && !is.null(VALUE) && (nchar(VALUE) > 0)),
                                                           `assoc.variable1` = expression(!is.null(FindNode(node$root, "oper.name"))),
                                                           `assoc.var1name` = expression("The extraction script is checked for existence of the field in the referent entity, if it is not NA"),
