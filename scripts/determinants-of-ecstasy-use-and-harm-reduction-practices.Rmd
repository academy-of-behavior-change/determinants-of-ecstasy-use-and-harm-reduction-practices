...
queries[[length(queries) + 1]] <-
  list(date = ymd("2018-07-01"),
       query = query_requiredConcepts(conceptName="Determinants of ecstasy use and harm reducation strategies",
...
However, extraction scripts contain very long lines when closely related entities are extracted in list form; in that case, their explanation and examples are placed behind the entities and values to extract, 

The second is to use an external editor. For extractors working in a Windows environment, Notepad++ is recommended; for extractors working in a Mac OS environment, BBEDit is recommended (extractors using a Linux distro probably already have their preferred text editors).
